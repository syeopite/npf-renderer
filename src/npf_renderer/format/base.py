...
class Formatter(helpers.CursorIterator):
    ...
    def _format_link(self, block):
        ...
        anchor_content_wrapper = dominate.tags.a(
            href=self.url_handler(block.url), cls="link-block-link"
        )
        ...
        if block.poster:
            ...
            if block.title:
                poster_container.add(
                    dominate.tags.div(
                        dominate.tags.span(dominate.util.text(block.title)),
        ...
        if site_name:
            ...
            if block.author:
                subtitles.add(
                    dominate.tags.span(dominate.util.text("|"), cls="site-name-author-separator")
                )
    ...
    def _format_video(self, block):
        ...
        if not video:
            if self.forbid_external_iframes and (
                block.embed_html or block.embed_url or block.embed_iframe
            ):
                return self._audiovisual_link_block_fallback(
    ...
    def _format_poll(self, block):
        ...
        with footer:
            ...
            expiration = datetime.datetime.utcfromtimestamp(
                block.creation_timestamp + block.expires_after
            )
            ...
            with dominate.tags.div(cls="poll-metadata"):
                ...
                if expiration > now:
                    ...
                    dominate.tags.span(
                        f"Remaining time: ",
                        HTMLTimeTag(str(remaining_time), datetime=duration_string),
                    )
                else:
                    formatted_expiration = expiration.strftime("%Y-%m-%dT%H:%M")
                    dominate.tags.span(
                        f"Ended on: ", HTMLTimeTag(str(expiration), datetime=formatted_expiration)
                    )
    ...
    def _audiovisual_link_block_fallback(
        self, block, title: str, description: str, site_name: str = None
    ):
        ...
        match self.current:
            case objects.text_block.TextBlock():
                if self.current.nest:
                    self.current_context_padding = _calculate_amount_to_pad_from_nested(
                        self.current
                    )
    ...
    def format(self):
            ...
            for layout in self.layout:
                if isinstance(layout, objects.layouts.Rows):
                    for row in layout.rows:
                        ...
                        if has_image:
                            ...
                            for item_index, item in enumerate(row_items):
                                ...
                                original_media_ratios.append(
                                    round((item[1].height / item[1].width) * 100, 4)
                                )
                            ...
                            for index, render_instruction_args in images_in_row:
                                row_items[index] = self._format_image(
                                    *render_instruction_args,
                                    len(images_in_row),
                                    override_padding=padding_ratio,
                                )
                    ...
                    self.post.add(
                        dominate.tags.div(
                            misc.format_ask(
                                self.url_handler, *layout_items, blog_attribution=layout.attribution
                            ),
                            cls="layout-ask",
                        )
                    )
        ...
        if self.has_render_error:
            raise exceptions.RenderErrorDisclaimerError(
                "Rendered post contains errors", rendered_result=self.post
            )
