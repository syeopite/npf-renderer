...
def helper_function(raw, answer, forbid_external_iframes=False, wrap_answer=True):
    has_error, formatted_result = format_npf(
        (raw,), pretty_html=True, forbid_external_iframes=forbid_external_iframes
    )
...
def test_can_format_basic_audio_block():
    return helper_function(
        mock_audio_blocks.basic_audio_block[0], mock_audio_blocks.basic_audio_block[2]
    )


def test_can_format_audio_block_with_title():
    return helper_function(
        mock_audio_blocks.audio_block_with_title[0], mock_audio_blocks.audio_block_with_title[2]
    )


def test_can_format_audio_block_with_metadata():
    return helper_function(
        mock_audio_blocks.audio_block_with_metadata[0],
        mock_audio_blocks.audio_block_with_metadata[2],
    )


def test_can_format_audio_block_with_poster():
    return helper_function(
        mock_audio_blocks.audio_block_with_poster[0], mock_audio_blocks.audio_block_with_poster[2]
    )
...
def test_can_format_audio_block_with_metadata_and_poster():
    return helper_function(
        ...
        mock_audio_blocks.audio_block_with_metadata_and_poster[2],
    )


def test_can_format_embedded_audio_block():
    return helper_function(
        mock_audio_blocks.audio_block_embed_spotify_test[0],
        mock_audio_blocks.audio_block_embed_spotify_test[2],
    )
