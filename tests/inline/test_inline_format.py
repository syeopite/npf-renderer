...
def helper_function(content, answer, url_handler=None):
    has_error, formatted_result = format_npf(
        content["content"], url_handler=url_handler, pretty_html=True
    )
...
def test_connected_back_to_back_inline():
    helper_function(
        {"content": connected_back_to_back_format_test[0]}, connected_back_to_back_format_test[1]
    )


def test_long_space_inbetween_render():
    helper_function(
        {"content": long_space_in_between_format_test[0]}, long_space_in_between_format_test[1]
    )


def test_second_has_lower_end_index():
    helper_function(
        {"content": second_has_lower_end_index_test[0]}, second_has_lower_end_index_test[1]
    )


def test_second_has_lower_end_index_2():
    helper_function(
        {"content": second_has_lower_end_index_test_2[0]}, second_has_lower_end_index_test_2[1]
    )


def test_overlapping_same_area():
    helper_function(
        {"content": overlapping_same_area_test_data[0]}, overlapping_same_area_test_data[1]
    )


def test_overlapping_same_start_different_end_data():
    helper_function(
        {"content": overlapping_same_start_different_end_data[0]},
...
def test_interrupted_same_indices_overlapping():
    helper_function(
        {"content": interrupted_same_indices_overlapping[0]},
        interrupted_same_indices_overlapping[1],
    )
...
def test_same_index_with_other_overlap():
    helper_function(
        {"content": same_index_with_other_overlap_test[0]}, same_index_with_other_overlap_test[1]
    )
...
def test_link_url_handler_replaced():
    ...
    helper_function(
        {"content": link_url_handler_test_data[0]},
        link_url_handler_test_data[2],
...
def test_excessively_out_of_bounds_end_single_op():
    helper_function(
        {"content": excessively_out_of_bounds_end_single_op_test[0]},
        excessively_out_of_bounds_end_single_op_test[1],
    )
...
def test_excessively_out_of_bounds_end_overlap():
    helper_function(
        {"content": excessively_out_of_bounds_end_overlap_test[0]},
        excessively_out_of_bounds_end_overlap_test[1],
    )
