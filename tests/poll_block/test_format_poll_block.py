...
def helper_function(raw, answer, callback=None):
    has_error, formatted_result = format_npf(
        (raw,), pretty_html=True, poll_result_callback=callback
    )
...
def test_basic_expired_poll_block():
    return helper_function(
        mock_poll_blocks.simple_poll_expired[0], mock_poll_blocks.simple_poll_expired[2]
    )
...
def test_basic_ongoing_poll_block():
    return helper_function(
        mock_poll_blocks.simple_ongoing_poll[0], mock_poll_blocks.simple_ongoing_poll[2]
    )
